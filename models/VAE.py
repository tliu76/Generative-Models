...
class BasicEncoder(nn.Module):
    def __init__(self, input_dim=784, hidden_dim=400, latent_dim=20):
        ...
        #############################################################################
        ## First linear layer mapps flattened input x-->hidden representation
        self.fc1 = nn.Linear(self.input_dim, self.hidden_dim)
         # Head 1: predicts the mean vector mu of q(z|x)
        self.fc_mu = nn.Linear(self.hidden_dim, self.latent_dim)
        ## Head 2: predicts the log-variance vector logvar of q(z|x)
    ...
    def forward(self, x):   
        ...
        #############################################################################
        h = F.relu(self.fc1(x))  ## # Pass input through the first linear layer, then apply ReLU 
        # to get the hidden representation
        mu = self.fc_mu(h)  ## mu mean of latent Gaussian
        logvar = self.fc_logvar(h)  ## logvar log-variance of latent Gaussian
...
class VAE(nn.Module):
    def __init__(self, input_dim=784, hidden_dim=400, latent_dim=256):
        ...
        ### decoder maps latent z --> reconstructed input x
    ...
    def reparameterize(self, mu, logvar):
        ...
        z = mu + std * eps  ## reparameterization trick: z = mu + sigma * epsilon, where epsilon ~ N(0,1)
    ...
    def encode(self, x):
        ...
        # pass through encoder --> get mu and logvar
